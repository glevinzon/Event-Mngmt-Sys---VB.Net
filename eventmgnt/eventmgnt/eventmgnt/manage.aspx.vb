...
Public Class manage
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        If Not IsPostBack Then
            ...
            If Request.QueryString("eventId") IsNot Nothing Then
                gvCustomers.DataSource = GetData("select eventId, userName, Name, DateTime, endDateTime, datediff(hh, DateTime , endDateTime) AS duration, Fee, Approval, created, modified from Events WHERE eventId= '" + Request.QueryString("eventId") + "'")
                gvCustomers.DataBind()
            Else
                gvCustomers.DataSource = GetData("select eventId, userName, Name, DateTime, endDateTime, datediff(hh, DateTime , endDateTime) AS duration, Fee, Approval, created, modified from Events WHERE userName='" + s_userName + "'")
                gvCustomers.DataBind()
            End If
    ...
    Protected Sub cmd_Search_Click(sender As Object, e As System.EventArgs)
        ...
        If CheckBox1.Checked = True Then
            ...
            gvCustomers.DataSource = GetData("select eventId, userName, Name, DateTime, endDateTime, datediff(hh, DateTime , endDateTime) AS duration, Fee, Approval, created, modified from Events WHERE userName = '" + s_userName + "' AND DateTime between'" + deyttaym + "' and '" + enddeyttaym + "' ")
            gvCustomers.DataBind()
        ElseIf CheckBox2.Checked = True Then
            gvCustomers.DataSource = GetData("select eventId, userName, Name, DateTime, endDateTime, datediff(hh, DateTime , endDateTime) AS duration, Fee, Approval, created, modified from Events WHERE userName = '" + s_userName + "' AND Name LIKE @Key")
            gvCustomers.DataBind()
        ElseIf CheckBox1.Checked = True And CheckBox2.Checked = True Then
            Dim deyttaym As String = DateTime.ParseExact(txt_sDateTime.Text, "M/d/yyyy HH:mm", CultureInfo.InvariantCulture).ToString("yyyy-MM-dd HH:mm:ss.mmm")
            Dim enddeyttaym As String = DateTime.ParseExact(txt_eDateTime.Text, "M/d/yyyy HH:mm", CultureInfo.InvariantCulture).ToString("yyyy-MM-dd HH:mm:ss.mmm")

            gvCustomers.DataSource = GetData("select eventId, userName, Name, DateTime, endDateTime, datediff(hh, DateTime , endDateTime) AS duration, Fee, Approval, created, modified from Events WHERE userName = '" + s_userName + "' AND DateTime between'" + deyttaym + "' and '" + enddeyttaym + "' AND Name LIKE @Key")
            gvCustomers.DataBind()
        Else
            gvCustomers.DataSource = GetData("select eventId, userName, Name, DateTime, endDateTime, datediff(hh, DateTime , endDateTime) AS duration, Fee, Approval, created, modified from Events WHERE userName = '" + s_userName + "'")
            gvCustomers.DataBind()
    ...
    Protected Sub OnRowDataBound(sender As Object, e As GridViewRowEventArgs)
        If e.Row.RowType = DataControlRowType.DataRow Then
            ...
            gvOrders.DataSource = GetData(String.Format("select top 3 * from Attendees where eventId='{0}'", customerId))
            gvOrders.DataBind()
        End If
    End Sub
