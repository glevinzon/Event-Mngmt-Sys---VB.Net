...
Public Class webadmin
    ...
    Protected Sub cmd_Search_Click(sender As Object, e As System.EventArgs)
        Dim strkey As String = txt_keyword.Text
        ...
        ElseIf CheckBox2.Checked = True Then
            gvCustomers.DataSource = GetData("select eventId, userName, Name, DateTime, endDateTime, datediff(hh, DateTime , endDateTime) AS duration, Fee, Approval, created, modified from Events WHERE Name LIKE '%" + strkey + "%'")
            gvCustomers.DataBind()
        ElseIf CheckBox1.Checked = True And CheckBox2.Checked = True Then
            Dim deyttaym As String = DateTime.ParseExact(txt_sDateTime.Text, "M/d/yyyy HH:mm", CultureInfo.InvariantCulture).ToString("yyyy-MM-dd HH:mm:ss.mmm")
            Dim enddeyttaym As String = DateTime.ParseExact(txt_eDateTime.Text, "M/d/yyyy HH:mm", CultureInfo.InvariantCulture).ToString("yyyy-MM-dd HH:mm:ss.mmm")

            gvCustomers.DataSource = GetData("select eventId, userName, Name, DateTime, endDateTime, datediff(hh, DateTime , endDateTime) AS duration, Fee, Approval, created, modified from Events WHERE DateTime between'" + deyttaym + "' and '" + enddeyttaym + "' AND Name LIKE '%" + strkey + "%'")
            gvCustomers.DataBind()
        Else
